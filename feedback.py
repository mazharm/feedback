...
def process_df_rawfeedback(feedback_df):
    """
...
def summarize_consolidate_df(d_f):
    """
    ...
    summaries_full = [list(zip(d_f['Summary'][i:i+chunk_size], d_f['Top Quotes'][i:i+chunk_size], d_f['Action Items'][i:i+chunk_size]))
    ...
    for chunk in summaries_full:
        ...
        action_items = oac.get_summary(a_c, PromptType.CONSOLIDATE_ACTION_ITEMS)
...
def summarize_workspace(workspace, d_f, consolidate):
    ...
    if (consolidate):
        summaries = summarize_consolidate_df(d_f)
    else:
        summaries = summarize_df(d_f)

    if (consolidate):
        print(f"summaries_df size:{d_f.shape[0]}, number of summaries: {len(summaries)}")
...
def process_df(d_f, min_text_length, process_raw_feedback, index):
    ...
    print(f"Processing shard #{index}. process_raw_feedback={process_raw_feedback}")
    df_summaries = pd.DataFrame(columns=summary_columns)
    ...
    for workspace in workspaces:
        ...
        if process_raw_feedback:
            workspace_df = process_df_rawfeedback(workspace_df)
...
def main(filename, shard_size, min_text_length, process_raw_feedback):
    ...
    df_summaries_full_list = [summary_result for summary_result in summary_results]
